...
resource "azurerm_virtual_machine" "vmfw1" {
  ...
  os_profile {
    ...
    custom_data    = "${join(",", list("storage-account=${var.Bootstrap_Storage_Account}", "access-key=${var.Storage_Account_Access_Key}", "file-share=${var.Storage_Account_File_Share}", "share-directory=${var.Storage_Account_File_Share_Directory}"))}"
  }
